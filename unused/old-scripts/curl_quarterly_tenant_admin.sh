...
# User: Tenant Admin - can upload cross-org data
# File: test_quarterly_250_rows.csv

API_BASE="http://localhost:8000/api/v1"

echo "🔐 Authenticating as tenant_admin (ceo@defaultrate.com)..."

# Get access token
TOKEN_RESPONSE=$(curl -s -X POST "$API_BASE/auth/login" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "username=ceo@defaultrate.com" \
  -d "password=ceo123" \
...
echo "\n✅ Quarterly bulk upload test completed for tenant_admin!"
