...
def get_or_create_test_user(db: Session):
    ...
    user = db.query(database.User).filter(database.User.email == test_email).first()
...
def create_company_from_data(row, db: Session, company_service: services.CompanyService):
...
def run_annual_prediction(company, ratios, ml_service, company_service, user_id, fiscal_year):
...
def main():
    ...
    # Load step data
    annual_data = load_step_data()
    
    # Initialize ML services
    print("Loading ML models...")
    ml_model_service = ml_service.MLModelService()
    ...
    try:
        ...
        for i, (_, row) in enumerate(annual_data.head(NUM_RECORDS).iterrows()):
            ...
            company = create_company_from_data(row, db, company_service)
            if not company:
            ...
            prediction = run_annual_prediction(
                company, ratios, ml_model_service, company_service, user.id, fiscal_year
            )
