...
# User: Morgan Stanley Org Admin - can upload org data
# File: test_quarterly_250_rows.csv

API_BASE="http://localhost:8000/api/v1"

echo "🔐 Authenticating as morgan_admin (risk.director@morganstanley.com)..."

# Get access token
TOKEN_RESPONSE=$(curl -s -X POST "$API_BASE/auth/login" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "username=risk.director@morganstanley.com" \
  -d "password=director123" \
...
echo "\n✅ Quarterly bulk upload test completed for morgan_admin!"
