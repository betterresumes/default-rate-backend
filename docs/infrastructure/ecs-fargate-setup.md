...
```yaml
# Core ECS Resources (example names)
...
- Parameter Store: Database URL, JWT secret, app config
```

> Note: Use Amazon ElastiCache for Redis in production. Running Redis as an ECS service is suitable for development/testing only.
...
REDIS_URL=redis://accunode-redis.xxxxxx.cache.amazonaws.com:6379

# Application Settings
ENVIRONMENT=production
DEBUG=false
SECRET_KEY=${SECRET_KEY_FROM_PARAMETER_STORE}
JWT_SECRET_KEY=${JWT_SECRET_FROM_PARAMETER_STORE}
  ...
  "containerDefinitions": [
    {
      ...
      "secrets": [
        {
          "name": "DATABASE_URL",
          "valueFrom": "arn:aws:ssm:us-east-1:ACCOUNT:parameter/accunode/production/database/url"
        },
        {
          "name": "SECRET_KEY",
          "valueFrom": "arn:aws:ssm:us-east-1:ACCOUNT:parameter/accunode/production/secret-key"
        },
        {
          "name": "JWT_SECRET_KEY",
          "valueFrom": "arn:aws:ssm:us-east-1:ACCOUNT:parameter/accunode/production/jwt-secret"
        }
      ],
      "logConfiguration": {
        "logDriver": "awslogs",
